...
"""makeMyFile
Created on Tues Jan 18 17:56:31 2022
@author: cariefrantz
@project: Teaching

Converts formatted autobiographical form in Google Docs
to pretty Canvas pages.

Arguments:  None

Example in command line:
    python makeMyFile.py
...
'''
First steps:
    1. Create file text in Google Docs using this template:
        https://docs.google.com/document/d/1fnkE50kwWSqGEyDwZ9Hem_oPoSMzB5icKp-oFK-PEvE/edit?usp=sharing
...
************
To Do:
    -Figure out how to save html directly from docs and parse it
    -Deal with figures
    -Debug the formatting... there are some issues with the way tables
        are found and formatted
...
delim_form_title = '<p>The Autobiographical Form</p>'
...
def importFile(filepath):
    with open(filepath) as fp:
        html = BeautifulSoup(fp, features='lxml')
    return html


def parse_sections(html, fname='auto'):
    '''

    Parameters
    ----------
    html : BeautifulSoup object
        html object as parsed by BeautifulSoup.
    fname : str, optional
        Whether or not to automatically generate the filename,
        or ask the user for input.
        The default is 'auto'.
        Any other value asks the user for input.

    Returns
    -------
    sections : dict of dicts
        Each section, with its filename and html
    ...
    for section in html.find_all('h1'):
        # Name the section
        title = section.get_text().strip()
        sections[title]={}
        if fname=='auto':
           # If the section has a numeral before it, use that
            sections[title]['fname'] = title.split('.')[0]
            
        else:
            # Ask the user to name the file
            sections[title]['fname'] = input(
                'Processing Section   "' + title + '''"   ...
What do you want to call the file? Enter in a word, no spaces, no characters.
E.g., TEACHING will produce the file TEACHING.html for this section.
   > ''')
        
        # Loop through each line of HTML starting at the top of the section,
        # end at the bottom of the section
        section_html = []
        nextSection = section
        while True:
            nextSection = nextSection.nextSibling
            
            # If the last section has been reached, finish the HTML
            if not nextSection:
                sections[title]['html'] = '\n'.join(section_html)
                break
            
            # If there is text, save it
            elif isinstance (nextSection, NavigableString):
                section_html.append(nextSection)
            
            # If it's a tag, determine whether it's the next section or not
            elif isinstance(nextSection, Tag):
                if nextSection.name == ('h1'):
                    sections[title]['html'] = '\n'.join(section_html)
                    break
                section_html.append(str(nextSection))      
            
    return sections


def build_section_content(section_html):
    
    # Grab all of the instances of h2
    headers = grab_content_between_tags(section_html, 'h2')
    
    if headers:
        #!! TO DO: Grab any content before the first header
        
        # Make the tab html & content boxes
        section_html = tab_html_head
        content_html = ''
        ...
        for h in headers:
            header = headers[h]
            # Get the tab title
            tab_title = header['firstline'].get_text().strip().split('.')[0]
            section_html = (section_html + '\n    <li><a href="'
                            + tab_html_prefix + tab_title + '">' + tab_title
                            + '</a></li>')
                
            # Add tab container div at the start of the content
            content_html = (content_html + '\n<div id = "'''
                            + tab_html_prefix + tab_title + '">\n')
            # Add content
            content_html = content_html + str(header['firstline']) + header['html']
            # Add tab container div to the end of the content
            content_html = content_html + '</div>\n\n'
            
        # Wrap up the html
        section_html = section_html + content_html
        
    return section_html
        

            
def grab_content_between_tags(html_to_search, tag):
    '''
    Parameters
    ----------
    html : str
        HTML (as a multiline string) to be searched.
    tag : str
        The html tag to search for, e.g., 'table', 'h1', 'p'.

    Returns
    -------
    instances : dict of dicts
        Each instance is numbered and saved in instances as a dict containing:
            'firstline' = the first line of the instance
            'html' = the content of the instance
    '''
    parsed_html = BeautifulSoup(html_to_search, features='lxml')
    instances = {}
    for i, instance in enumerate(parsed_html.find_all(tag)):
        # Save the instance
        instances[i] = { 'firstline' : instance }
        
        # Loop through subsequent code and grab all of the content
        content_html = []
        next_instance = instance
        while True:
            next_instance = next_instance.nextSibling
        
            # If the last instance has been reached, finish the HTML
            if not next_instance:
                instances[i]['html'] = '\n'.join(content_html)
                break
            
            # If there is text, save it
            elif isinstance (next_instance, NavigableString):
                content_html.append(next_instance)
            
            # If it's a tag, determine whether it's the next instance or not
            elif isinstance(next_instance, Tag):
                if next_instance.name == (tag):
                    instances[i]['html'] = '\n'.join(content_html)
                    break
                content_html.append(str(next_instance))       
    ...
    ### Old (and working...mostly) ######################################
def old_code():
    # Select files (UI)
    file, dirPath = select_file()
...
if __name__ == '__main__': 
    ...
    # Select files (UI)
    file, dirPath = select_file()
    # First, read in and reformat the file
    print('Processing ' + file)
    lines = read_file(file)
    # Delete any spans
    html = delete_tags(lines,['span'],flex=True)
    # Delete any text before 'The Autobiographical Form'
    html = html[find_indices(html, delim_form_title)[0]:]
    # Save the HTML
    fname = file.split('.')[0]
    with open(fname + file_sfx_clean,'wb') as f:
        for line in html:
            f.write(line.encode('utf-8'))
    
    # Read in the file as a BeautifulSoup object (parses the HTML)
    html = importFile(fname + file_sfx_clean)
    ...
    # Loop through each section, process, and save
    for section in sections:
        # Convert h2 to tabs
        section_html = build_section_content(sections[section]['html'])
        
        # Save the html
        with open(sections[section]['fname'] + '.html', 'wb') as f:
